...
#include "../../fio/fio/lib/pow2.h"
#include "../../fio/fio/optgroup.h"

std::ofstream my_fd;

void open_and_write_in_file(std::string name){
	my_fd.open("/home/user/logi/log_libaio1.txt",std::ios_base::app);
	my_fd<<name<<std::endl;
	my_fd.flush();
	my_fd.close();
}
...
struct libaio_data {
	...
	struct io_event *aio_events; //указатель на результат запроса libaio
	struct iocb **iocbs; // указатель на массив указателей структуры описывающей один запрос в libaio
	struct io_u **io_us;
	...
	/* Основной кольцевой буфер. «head» увеличивается в _queue (), а «tail» увеличивается в _commit ().
	 Мы держим «в очереди», чтобы мы знали, заполнено или пустое кольцо, когда «head» == 'tail'.
	  «entries» - это размер кольца,
	 а «is_pow2» - это просто оптимизация использования 
	 AND вместо модуля, чтобы получить остаток от приращения кольца.*/
	int is_pow2;
...
{
	open_and_write_in_file("fio_libaio_prep");
...
{
	open_and_write_in_file("fio_libaio_event");
	struct libaio_data *ld = (libaio_data*)(td->io_ops_data);//kek
	struct io_event *ev;
	struct io_u *io_u;
...
{
	open_and_write_in_file("fio_libaio_getevents");
	struct libaio_data *ld = (libaio_data*)td->io_ops_data;
	struct libaio_options *o = (libaio_options*)td->eo;
...
{
	open_and_write_in_file("fio_libaio_queue");
	struct libaio_data *ld = (libaio_data*)td->io_ops_data;
...
static int fio_libaio_commit(struct thread_data *td)
{
	open_and_write_in_file("fio_libaio_commit");
	struct libaio_data *ld = (libaio_data*)td->io_ops_data;
	struct iocb **iocbs;
	struct io_u **io_us;
	struct timespec ts;
	...
	do {
		...
		io_us = ld->io_us + ld->tail;//волшебная арифметика с закольцованным буфером
		iocbs = ld->iocbs + ld->tail;

		ret = io_submit(ld->aio_ctx, nr, iocbs);//сискол libaio который отправляет 
		if (ret > 0) {
...
{
	open_and_write_in_file("fio_libaio_cancel");
	struct libaio_data *ld = (libaio_data*)td->io_ops_data;
...
{
	open_and_write_in_file("fio_libaio_cleanup");
	struct libaio_data *ld = (libaio_data*)td->io_ops_data;
...
{
	open_and_write_in_file("fio_libaio_init");
	struct libaio_options *o = (libaio_options*)td->eo;
...
static void fio_init fio_libaio_register(void)
{
	...
	open_and_write_in_file("fio_libaio_register");
	register_ioengine(&ioengine);
}

static void fio_exit fio_libaio_unregister(void)
{
	open_and_write_in_file("fio_libaio_unregister");
...
void get_ioengine(struct ioengine_ops **ioengine_ptr){// функция возвращает через свой параметр указатель на структуру
    *ioengine_ptr = &ioengine;
    ioengine.name           = "libaio";
    ioengine.version        = FIO_IOOPS_VERSION;
    ioengine.init           = fio_libaio_init;
    ioengine.prep		    = fio_libaio_prep;
    ioengine.queue          = fio_libaio_queue;
    ioengine.commit         = fio_libaio_commit;
    ioengine.cancel		    = fio_libaio_cancel;
    ioengine.getevents      = fio_libaio_getevents;
    ioengine.event          = fio_libaio_event;
    ioengine.cleanup        = fio_libaio_cleanup;
    ...
    ioengine.option_struct_size	= sizeof(struct libaio_options);
};
}
